...
async fn it_reads_the_project_from_an_absolute_path() {
    ...
    warp.prepare(&Label::all()).await.unwrap();
...
async fn build_dependency_graph_from_workspace() {
    ...
    let mut dep_graph = warp.dep_graph;
    ...
    let mut expected = vec![
        "//warp.build/test/toolchains:caramel".to_string(),
        "//warp.build/toolchains:erlang".to_string(),
...
async fn build_dependency_graph_from_workspace_and_scope() {
    ...
    let workspace: Workspace =
        WorkspaceBuilder::build(root.clone(), None, None).unwrap();
    let mut warp = LocalWorker::from_workspace(workspace);
    warp.prepare(&Label::new("//b/c:lib")).await.unwrap();
