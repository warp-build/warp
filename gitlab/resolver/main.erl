...
* resolver resolver https://gitlab.com/user/repo <git-ref>
* prepare .
...
resolve(Url0, Ref0) -> 
  ...
  RepoName = case Tokens of
               [_Http, _Gitlab, _Username, Repo] -> Repo
             end,

  Prefix = << RepoName/binary, "-", Ref/binary >>,
  #{
    version => 0,
    archive => #{ 
                 url => <<Url/binary, "/-/archive/", Ref/binary, "/", Prefix/binary, ".tar.gz">>,
                 strip_prefix => Prefix
                }
   }.
