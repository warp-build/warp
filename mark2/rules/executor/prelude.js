...
Warp.Signatures.compute = (inputSignature) => {
  ...
  const ruleConfig = Object.fromEntries(Object.entries(rule.cfg)
    .map( ([k, type]) => {
      const value = signature.cfg[k] === undefined ? rule.defaults[k] : signature.cfg[k];
      if (value === undefined) panic(`Expected signature  ${signature.target}  to have config key '${k}' (of type ${type}) but it was not present in your Build.toml, and it doesn't have a default value.`);
      return [k, value];
    }));
  ...
  const ctx = {
    ...
    env: () => ({
      host: {
        ...
        os: signature.env.platform.endsWith("darwin") ? "darwin" :
            signature.env.platform.endsWith("linux-gnu") ? "linux" :
            signature.env.platform.endsWith("win32") ? "win32" : "unknown",
      }
    }),
    ...
    action: () => ({
      copy: ({src, dst}) => _actions.push({ ffi: "op_ctx_actions_copy", data: {target, src, dst} }),

      declareOutputs: outs => _actions.push({ ffi: "op_ctx_actions_declare_outputs", data: {target, outs} }),

      declareRunScript: (runScript, opts = { env: {} }) => _actions.push({ ffi: "op_ctx_actions_declare_run_script", data: {target, runScript, env: opts.env}}),

      download: ({url, sha1, output}) => _actions.push({ ffi: "op_ctx_download", data: {target, url, sha1, output}}),

      exec: ({env = {}, cmd, args, cwd, needsTty = false}) => _actions.push({ffi: "op_ctx_actions_exec", data: {target, cmd, args, cwd, env, needsTty}}),

      extract: ({src, dst}) => _actions.push({ffi: "op_ctx_extract", data: {target, src, dst}}),

      runShell: ({script, env = {}, needsTty = false}) => _actions.push({ffi: "op_ctx_actions_run_shell", data: {target, script, env, needsTty}}),

      setPermissions: ({file, executable}) => _actions.push({ffi:"op_ctx_set_permissions", data: {target, file, executable}}),

      writeFile: ({data, dst}) => _actions.push({ffi:"op_ctx_actions_write_file", data: {target, data, dst}}),

      verifyChecksum: ({file, sha1}) => _actions.push({ffi:"op_ctx_verify_checksum", data: {target, file, sha1}}),
    }),
  };
  ...
  if (_provides !== null) ffi("op_ctx_declare_provides", {target, provides: _provides});
...
const check_config = cfg => {
  return Object.fromEntries(Object.entries(cfg).map(([k, t]) => {
    if (Array.isArray(t)) {
      if (t.length == 0) panic(`Cfg map for rule ${name} found an empty list. Did you mean to use   [target()]   ?`);
...
Warp.Rule = spec => {
  ...
  if (Object.entries(cfg).length == 0) panic(`Config map for rule ${name} is empty! Try adding a   name: target()   key?`);
