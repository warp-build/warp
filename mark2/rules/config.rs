...
impl RuleConfig {
    ...
    pub fn get_string_list(&self, key: &str) -> Result<Vec<String>, RuleConfigError> {
        ...
        if let CfgValue::List(elements) = entry {
            ...
            Err(RuleConfigError::KeyHadWrongType {
                expected: CfgValueType::List(Box::new(CfgValueType::Target)),
                found: entry.clone(),
                key: key.to_string(),
            })
        }
    ...
    pub fn get_target_list(&self, key: &str) -> Result<Vec<Target>, RuleConfigError> {
        let entry = self
            .config
            .get(key)
            .ok_or_else(|| RuleConfigError::MissingKey(key.to_string()))?;
        ...
        if let CfgValue::List(elements) = entry {
            let mut targets = vec![];
            for el in elements {
                if let CfgValue::Target(l) = el {
                    targets.push(l.clone());
                } else if let CfgValue::String(s) = el {
                    targets.push(s.parse::<Target>().map_err(RuleConfigError::TargetError)?);
                } else {
                    return Err(RuleConfigError::UnexpectedValueTypeInList {
                        expected: CfgValueType::Target,
                        found: el.clone(),
                        key: key.to_string(),
                    });
                }
...
impl Serialize for CfgValue {
        ...
        S: serde::Serializer,
    {
        match self {
            ...
            CfgValue::Target(l) => serializer.serialize_str(&l.to_string()),
        ...
        fn try_from(
            ...
            match &mut json {
                serde_json::Value::String(string) => match type_ {
                    ...
                    CfgValueType::Target => Ok(CfgValue::Target(
                        string.parse().map_err(RuleConfigError::TargetError)?,
                    )),
    ...
    impl From<CfgValue> for serde_json::Value {
        fn from(val: CfgValue) -> Self {
            match val {
                ...
                CfgValue::Target(target) => serde_json::Value::String(target.to_string()),
                CfgValue::File(path) => {
