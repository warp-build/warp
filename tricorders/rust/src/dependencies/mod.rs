...
pub struct DependencyManager {
    ...
    crates: Arc<Crates>,
    dependencies: DashMap<String, Package>,
}
...
impl DependencyManager {
    ...
    fn consolidate_dependencies(&self) {
        let deps: DashMap<String, Package> = DashMap::default();
        for lockfile in self.cargo_locks() {
            for package in lockfile.packages() {
                deps.insert(package.name().to_string(), package.clone());
            }
        }
    ...
    pub fn dependencies(&self) -> &DashMap<String, Package> {
        &self.dependencies
    }
}
...
"#,
        ...
        let dep = deps.get("async-stream").unwrap();
        assert_eq!(dep.name(), "async-stream");
        assert_eq!(dep.version(), "0.3.3");
        assert_eq!(dep.dependencies().len(), 2);

        let dep = deps.get("anyhow").unwrap();
        assert_eq!(dep.name(), "anyhow");
        assert_eq!(dep.version(), "1.0.68");
