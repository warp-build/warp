...
do_locked_dependencies(WorkspaceRoot, Projects) ->
    LockedDeps =
        case file:consult(path:join(WorkspaceRoot, "rebar.lock")) of
            {ok, Lock} ->
                case proplists:to_map(Lock) of
                    #{"1.2.0" := X} -> X;
                    _ -> []
                end;
            _ ->
    ...
    FinalLockedDeps =
        case LockedDeps of
            [] ->
                {ok, NewLock} = file:consult(
                    path:join(WorkspaceRoot, ".warp/_rebar_tmp/rebar.lock")
                ),
...
find_all_rebar_projects(Root0) when is_binary(Root0) ->
    ...
    MixProjects = filelib:fold_files(
        Root, "hex_metadata\.config$", true, fun read_hex_metadata/2, #{}
    ),
...
do_extract_deps(Projects) ->
    ...
    uniq(
        lists:flatmap(
            fun
                (_Pkg = #{deps := Deps, profiles := #{test := #{deps := TestDeps}}}) when
                    Deps =/= [], is_map(Deps), TestDeps =/= [], is_map(TestDeps)
                ->
...
collect_and_choose_newer([], Acc) ->
    maps:from_list(lists:sort(maps:values(Acc)));
